...
<tr>
  ...
  <cfloop INDEX="x" FROM="1" TO="#DaysInMonth(prev_date)#">
  	 ...
  	 <cfquery name="appshow" datasource="#settings.dsn#">
		...
		FROM 	cms_eventcal
		WHERE	start_date = <cfqueryparam cfsqltype="cf_sql_date" value="#Createdate(theYear,theMonth,X)#">
		order by time_start
	</cfquery>
    ...
    <cfoutput>
      ...
      <cfelse>
        <td class="text-center site-color-1-bg">
        	 ...
        	 <cfelse>
          	<a href="allday.cfm?da=#x#&amp;mo=#datepart("m",prev_date)#&amp;ye=#datepart("yyyy",prev_date)#" class="smcalday text-white">#x#</a>
          </cfif>
        </td>
      </cfif>
    </cfoutput>
