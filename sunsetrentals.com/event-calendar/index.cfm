...
<div class="event-calendar">
	...
	<div class="alert site-color-1-bg" style="overflow:hidden;">
      ...
      <label for="theselmo" class="hidden">theselmo</label>
	    <select name="theselmo" id="theselmo" onChange="gotomo()">
	      <cfoutput>
	  ...
	  <form name="selyr" style="text-align: left;">
  	  <label for="theselyr" class="hidden">theselyr</label>
	    <select name="theselyr" id="theselyr" onChange="gotoyr()">
	      <cfloop index="a" from="1" to="1" step="-1">
	        <cfset b = laye - a>
	    ...
	    <table class="table table-bordered big-cal">
		   ...
		   <tr>
		      ...
		      <cfloop index="X" from="1" to="#DaysInMonth(DateOb)#">
		        ...
		        FROM 	cms_eventcal
		        WHERE	start_date = <cfqueryparam cfsqltype="cf_sql_date" value="#Createdate(url.ye,url.mo,X)#">
		        order by time_start
		        </cfquery>
		        <td class="hdnvc<cfif day(now()) eq X and month(now()) eq url.mo> site-color-1-bg</cfif>" width="14.28%">

		          <cfif appshow.recordcount EQ 0>

		          	 <cfoutput><span class="label <cfif day(now()) eq X and month(now()) eq url.mo>text-white<cfelse>text-black</cfif>">#X#</span></cfoutput>

		          <cfelse>

		            <cfoutput>
		  			    <a href="allday.cfm?da=#x#&mo=#url.mo#&ye=#url.ye#">
		  			      <span class="label <cfif day(now()) eq X and month(now()) eq url.mo>text-white<cfelse>text-black</cfif>">#X#</span>
		  			    </a>
		  			  </cfoutput>
		            ...
		            <cfoutput query="appshow">
		              <label class="label site-color-3-bg site-color-3-lighten-bg-hover">
		                <a href="eventshow.cfm?id=#id#" class="text-white text-white-hover">#CV#. #event_title#</a>
		              </label>
		              <cfset CV=CV + 1>
		            </cfoutput>
