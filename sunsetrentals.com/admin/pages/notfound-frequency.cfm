...
<cfif isdefined('url.flush')>
   ...
   <cfquery dataSource="#dsn#">
     <!---DELETE FROM notfound---> <!--- needs DROP privileges as of new mysql version, but DROP privileges has been disabled due to security reasons. Please contact Bob for more info. --->
...
<cfif isdefined('MarkAsFixed')>
  ...
  <cfquery name="GetFrequency" dataSource="#application.dsn#">
  	select 
  	count(thepage) as countTP,  thepage, id, notes, max(thedate) as maxdate
	from notfound 
