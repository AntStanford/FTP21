...
<cfoutput>
	<div id="hiddenDeposit" style="display:none;" data-cost="#json.breakdown.total / 2#"></div>
		        ...
		        <div class="alert-success amount-without-insurance" <cfif structKeyExists( url, 'ti' ) and url.ti is 'add_insurance'>style="display:none"</cfif>>
		            <h4 class="panel-title">
		                <a data-toggle="collapse" href="##collapse1" id="togglem" <cfif isDefined('url.expanded') AND url.expanded EQ 'true'> aria-expanded="true"</cfif>>
		                  <div class="property-cost-list-total"> <span id="costRowShowHide" class="glyphicon glyphicon-plus"></span> <strong><cfif val( numdaysout ) gte 60>Total<cfelse>Total</cfif></strong> <span class="text-right" id="summarytotal">#DollarFormat(json.breakdown.total)#</span></div>
		                </a>
		            </h4>
				...
				<div class="alert alert-info">
					<p><input type="radio" name="tchoice" class="tchoice" id="tchoice_total" value="full" <cfif isDefined('url.paychoice') and url.paychoice is 'full'>checked="checked"</cfif> onclick="updateSummaryOfFees()"/> 
					  ...
					  <p><input type="radio" name="tchoice" class="tchoice" id="tchoice_half" value="half"  <cfif (isDefined('url.paychoice') and url.paychoice is 'half')>checked="checked"</cfif> onclick="updateSummaryOfFees()"/> 
						<label for="halftotal">Pay Deposit <span id="halftotalr">#dollarFormat( halftotal )#</span></label></p>
				  </div>
<!---
...
--->
	...
	<script type="text/javascript">
	   <cfif val( numdaysout ) gte 60 and isDefined('url.paychoice') and url.paychoice is 'half' AND not StructKeyExists(json,'promoCode')>
	   	// console.log('here1');
		    ...
		    <cfif ListFind('216.99.119.254,209.188.44.35', CGI.REMOTE_ADDR)>
		    $('input##Total').val("#halftotal#");
		    <cfelse>
		    $('input##Total').val("#halftotal#");
		    </cfif>
			
			$('input##tripinsuranceamount').val("#json.breakdown.travelInsurance#");
			$('##BookingValue').val('#halftotal#');
			$('##payHalf').val(true);
		<cfelse>
			// console.log('here2');
