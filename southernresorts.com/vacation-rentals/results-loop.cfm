...
<!--- <cfif ListFind('216.99.119.254', CGI.REMOTE_ADDR)><cfdump var="#session.booking.getResults#"></cfif> --->
<!---
<cfif ListFind('216.99.119.254', CGI.REMOTE_ADDR)>
  <cfoutput>#session.booking.getResults.recordcount#</cfoutput>
<!---  <cfdump var="#session.booking.getResultsPre#"> --->
<!---  <cfdump var="#form#"> --->
<!--- <cfoutput>#session.booking.getResults.recordcount#</cfoutput> --->
     <!---  <cfdump var="#session.booking.getResultsPre#">
...
<!--- <cfoutput query="session.booking.getResultsPre">
 #currentrow# - #name#<br>
</cfoutput>
<br><br>
<cfoutput query="session.booking.getResultsPre" group="resortname">
 #currentrow# - #name#<br>
</cfoutput> --->
  <!--- <cfoutput>
    ...
    From session.booking.getResultsPre
    Where type = 'Home'
    </cfquery>
  ...
  <p>Total PRE Recordcount : #session.booking.getResultsPre.recordcount#</p>
  ...
  <br>
  <p>Total Recordcount : #session.booking.getResults.recordcount#</p>
 ...
 --->
<cfif ListFind('70.40.113.45,216.99.119.254', CGI.REMOTE_ADDR)>
<!---   <cfdump var="#session.booking.getResultsPre#"> --->
    <!---   <cfdump var="#session.booking.getResultsPre#"> --->
    <!--- <cfdump var="#ListLen(session.booking.unitCodeList)#"> --->
    <!---   <cfdump var="#session.booking.getResultsPost#" abort="true"> --->
  <!---   <cfdump var="#session.booking#"> --->
    <cfif isdefined('session.booking.unitCodeList')><p>ICND EYES ONLY: <cfoutput>#ListLen(session.booking.unitCodeList)#</cfoutput> results</p></cfif>
</cfif>

<!--- <cfif ListFind('216.99.119.254,70.40.100.144', CGI.REMOTE_ADDR)><cfdump var="#session.booking.getResultsPre#"></cfif> --->
...
<cfif isdefined('session.booking.unitCodeList') and ListLen(session.booking.unitCodeList) gt 0 and isdefined('session.booking.getResults')>
    ...
    <cfif isdefined('session.booking.searchByDate') and session.booking.searchByDate and isdefined('session.specialid') and len(session.specialid) and session.specialid gt 0>
      ...
      <cfquery name="checkForSpecials" dataSource="#settings.dsn#">
        select id from cms_specials where #createodbcdate(session.booking.strcheckin)#
  ...
  <cfelseif isdefined('session.booking.searchByDate') and session.booking.searchByDate and isdefined('session.booking.strcheckin')>
    ...
    <cfquery name="checkForSpecials" dataSource="#settings.dsn#">
      ...
      #createodbcdate(session.booking.strcheckin)# between startdate and enddate and
      #createodbcdate(session.booking.strcheckin)# between allowedBookingStartDate and allowedBookingEndDate
      and active = 'yes'
    </cfquery>
    <cfif checkForSpecials.recordcount gt 0>
	...
	<div class="results-list-properties <cfif StructKeyExists(request,'resortContent')>resorts-list-properties</cfif>">
      ...
      <cfif url.loopend gt cookie.numresults>
		...
		<cfif isDefined("request.resortContent") OR getunitinfo.typename EQ "" OR isDefined("session.booking.specialID")>
...
--->
  	      <li class="<cfif cgi.script_name eq "/destination-page.cfm">col-xlg-4 col-lg-4 col-md-6 col-sm-6<cfelse>col-xlg-4 col-lg-6 col-md-6 col-sm-6 <cfif cookie.numresults EQ 1>flex-result-single </cfif>results-list-col</cfif>">
...
<!--- 						  <cfif not isDefined("request.destination")> --->
			  	            <a href="javascript:;" class="results-list-property-favorite add-to-favs add-to-fav-results" <cfif not getunitinfo.isproperty AND not isDefined("request.resortContent") AND not  isDefined("session.booking.specialID")>style="display: none;"</cfif>>
...
<!--- 						  </cfif> --->
			  	            <!--- 				</cfif> --->
										  <div class="owl-carousel owl-theme owl-quick-gallery">
													...
													<cfif isDefined("request.resortcontent") or LEN(getunitinfo.typename) eq 0 or cgi.script_name eq '/layouts/special.cfm' or (isDefined("session.booking.specialID") and LEN(session.booking.specialID))>


<!--- 														<cfif CGI.REMOTE_ADDR eq "216.99.119.254" or CGI.REMOTE_ADDR eq "99.1.177.220" or CGI.REMOTE_ADDR eq "173.93.73.19"> --->

															<cfif mobileDetect>
<!---
																<script>console.log('mobdetect-resorts');

																</script>
--->

																<cfquery datasource="#settings.dsn#" name="qryImages">
															 select * from track_properties_images where propertyid = <cfqueryparam CFSQLType="CF_SQL_INTEGER" value="#getunitinfo.propertyID#"> limit 3
														    </cfquery>
...
<!---
														<cfif CGI.REMOTE_ADDR eq "173.93.73.19">
		      	 	              <!---shows here--->
																<script>console.log('jtest01-res');</script>
														<cfelse>
														</cfif>
--->
...
<!--- 														<cfif CGI.REMOTE_ADDR eq "216.99.119.254" or CGI.REMOTE_ADDR eq "99.1.177.220" or CGI.REMOTE_ADDR eq "173.93.73.19"> --->
															...
															<cfif mobileDetect>
<!---
																<script>console.log('mobdetect');

																</script>
--->
																<cfquery datasource="#settings.dsn#" name="qryImages">
																	 select * from track_nodes_images where nodeid = <cfqueryparam CFSQLType="CF_SQL_INTEGER" value="#getunitinfo.nodeID#"> limit 3
																</cfquery>
															<cfelse>
																<cfquery datasource="#settings.dsn#" name="qryImages">
...
<cfif CGI.REMOTE_ADDR eq "173.93.73.19">
		      	 	              <!---shows here--->
																<script>console.log('jtest02-double');</script>
														<cfelse>
														</cfif>
--->
...
<!--- 						<span class="results-list-property-info-type">#getUnitInfo.type#</span> hidden per client req--->
  	          ...
  	          <div class="results-list-property-info-wrap">
                  ...
                  <cfif session.booking.searchByDate>
	                    ...
	                    <cfelse>
          							...
          							<cfif not isDefined("getunitinfo.resortproplist")>
          								...
          								<cfset returns = application.bookingObject.getDetailRates2(getUnitInfo.propertyid,session.booking.strcheckin,session.booking.strcheckout)>
										...
										<script type="text/javascript">
										  $.ajax(
											{
												url : "/vacation-rentals/ajax/get-srp-rates.cfm?propertyid=#getUnitinfo.propertyid#&checkin=#session.booking.strcheckin#&checkout=#session.booking.strcheckout#",
												type: "POST",
          							...
          							<cfelse>
										...
										var apiresponse = "";
										  <cfloop list="#getunitinfo.resortproplist#" index="currentprop">
        										<cfif currentprop NEQ "a">
												  $.ajax(
													{
														url : "/vacation-rentals/ajax/get-srp-rates3.cfm?propertyid=#currentprop#&checkin=#session.booking.strcheckin#&checkout=#session.booking.strcheckout#&resortproplist=getunitinfo.resortproplist",
														type: "POST",
														success: function(data)
														{
														 ...
														 if( total_#getUnitInfo.propertyid# > total2_#getUnitInfo.propertyid#){
															total_#getUnitInfo.propertyid# = total2_#getUnitInfo.propertyid#;
															 ...
															 $.ajax(
																{
																	url : "/vacation-rentals/ajax/get-srp-rates2.cfm?propertyid=#currentprop#&checkin=#session.booking.strcheckin#&checkout=#session.booking.strcheckout#&resortproplist=getunitinfo.resortproplist",
																	type: "POST",
																	success: function(data)
																	{
																	$('##quote_#getUnitInfo.propertyid#').html(data);
										...
										<!---
          								...
          								<cfloop list="#getunitinfo.resortproplist#" index="currentprop">
        										<cfif currentprop NEQ "a">
        											<cfset returns = application.bookingObject.getDetailRates2(currentprop,session.booking.strcheckin,session.booking.strcheckout,getunitinfo.resortproplist)>
  	            ...
  	            <ul class="results-list-property-info">
  	              ...
  	              <cfif getUnitInfo.bedrooms gt 0>
                  	<li><!--- <i class="fa fa-bed site-color-1" aria-hidden="true" data-toggle="tooltip" data-placement="bottom" title="Bedrooms"></i> --->Bedrooms: <cfif isDefined("getUnitInfo.bedroomsrange") and getUnitInfo.bedroomsrange contains "-">#replace(getUnitInfo.bedroomsrange,"abc","")#<cfelse>#getunitinfo.bedrooms#</cfif></li>
                  ...
                  <cfif getUnitInfo.sleeps gt 0>
                  	<li><!--- <i class="fa fa-users site-color-1" aria-hidden="true" data-toggle="tooltip" data-placement="bottom" title="Guests"></i> ---><!--- #getUnitInfo.sleeps# Guests --->Sleeps: <cfif isDefined("getUnitInfo.sleepsrange")>#replace(getUnitInfo.sleepsrange,"abc","")#<cfelse>#getunitinfo.sleeps#</cfif></li>
									...
									<li style="display: block;"><cfif isdefined('getUnitInfo.distanceToBeach') and LEN(getUnitInfo.distanceToBeach) and LEN(getUnitInfo.isProperty) and getUnitInfo.isProperty eq 1>Distance To Beach: #getUnitInfo.distanceToBeach#<cfelse>&nbsp;</cfif></li>
        ...
        <!---THIS IS THE FLEX DAY SECTION--->
	      <cfif 1 EQ 0 AND session.booking.getresults.recordcount LT 10 AND not isDefined("request.destination") AND not isDefined("request.resortid") AND session.booking.searchbydate AND not isDefined("session.booking.specialID")>
				  ...
				  <cfinclude template="results-search-query-partial.cfm">
					...
					<cfquery name="getPartialResults" dataSource="#settings.booking.dsn#">
						...
						where track_properties.id in (<cfqueryparam value="#session.booking.partialResultsList#" cfsqltype="cf_sql_varchar" list="true">)
	    				and track_properties.id not in  (<cfqueryparam value="#session.booking.unitCodeList#" cfsqltype="cf_sql_varchar" list="true">)
              <cfif structKeyExists(form,'RESORTS') and listlen(form.RESORTS)>
                and track_properties.nodeId in(<cfqueryparam value="#form.RESORTS#" cfsqltype="cf_sql_integer" list="true">)
              </cfif>
							order by rand()
						limit 5
...
<cfelseif not isDefined("request.destination") AND not isDefined("request.resortid") AND session.booking.searchbydate AND not isDefined("session.booking.specialID") AND 0 EQ 1>
...
<cfif isdefined('session.booking.unitCodeList') and ListLen(session.booking.unitCodeList) eq 0 and isdefined('session.booking.getResults') and session.booking.getResults.recordcount eq 0>
  <cfinclude template="results-loop-no-results.cfm">
</cfif>
