...
<cfset variables.pratikIP = '150.107.241.109' />
<cfset variables.saraIP = '68.44.158.31' />

<cfif cgi.remote_addr neq '150.107.232.57' and cgi.remote_addr neq variables.adouteIP and cgi.remote_addr neq variables.jimIP and cgi.remote_addr neq variables.coleIP and cgi.remote_addr neq variables.pratikIP and cgi.remote_addr neq variables.saraIP>
    <cfabort>
</cfif>
        ...
        <table class="table table-striped table-hover table-condensed">
            ...
            <cfoutput>
                <cfloop from="1" index="i" to="#arrayLen(arrMatchingPaths)#">
                    ...
                    <tr>
                        <td>#i#</td>
                        <td>#arrMatchingPaths[i]#<br>

                            <cffile action="read" file="#arrMatchingPaths[i]#" variable="exportthing">

                            <cfset exportthingForFinding = exportthing>
                            <cfset findTheThing = findNoCase(searchcriteria,exportthingForFinding)>
                            <cfset previousLines = 1>

                            <cfloop condition="findTheThing greater than 0">

                            <cfoutput>

                                <cfset SearchIN = left(exportthingForFinding,findTheThing)>
                                <cfset exportthingForFinding = right(exportthingForFinding,len(exportthingForFinding)-findTheThing)>
                                <cfset removeLineBreaks = replace(SearchIN,chr(10),'','all')>
                                <cfset howManyLines = len(SearchIN) - len(removeLineBreaks)>
                                <cfset previousLines = howManyLines + previousLines>
                                #previousLines# |
