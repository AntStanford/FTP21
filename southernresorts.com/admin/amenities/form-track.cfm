...
<cfquery name="getAmenityCategories" dataSource="#settings.booking.dsn#">
	select distinct amenityGroupName from track_properties_amenities order by amenityGroupName
</cfquery>
...
<cfif isdefined('url.id')>
	...
	<cfquery name="getCategoryAmenities" dataSource="#settings.booking.dsn#">
		select distinct amenityName from track_properties_amenities where amenityGroupName = <cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#getinfo.category#">
	</cfquery>
</cfif>

<cfif isdefined('url.category')>
	<cfquery name="getCategoryAmenities" dataSource="#settings.booking.dsn#">
		select distinct amenityName from track_properties_amenities where amenityGroupName = <cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#url.category#">
	</cfquery>
</cfif>
					...
					<div class="controls">
						<select name="category" style="width:300px" onchange="javascript:this.form.submit();">
							...
							<cfloop query="getAmenityCategories">
								<option <cfif (isdefined('url.category') and url.category eq getAmenityCategories.amenityGroupName) or (isdefined('url.id') and getinfo.category eq getAmenityCategories.amenityGroupName and !isDefined('url.category'))>selected</cfif>>#amenityGroupName#</option>
							</cfloop>  					
						</select>
					</div>
				</div>      
            
            <cfif isdefined('url.id') or isDefined('url.category')>
					<div class="control-group">
						...
						<div class="controls">
							<select name="amenity" style="width:300px">
								...
								<cfloop query="getCategoryAmenities">
									<option value="#upperFirst(amenityName)#" <cfif isdefined('url.id') and getinfo.title eq getCategoryAmenities.amenityName>selected</cfif>>#upperFirst(amenityName)#</option>
								</cfloop>  					
							</select>
						</div>
					</div>
